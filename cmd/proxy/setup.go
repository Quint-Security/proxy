...
func runSetup(args []string) {
	...
	envContent := fmt.Sprintf(`# Quint proxy environment — auto-generated by quint setup
# CA trust (always safe — doesn't break anything if proxy is stopped)
export SSL_CERT_FILE=%s
export NODE_EXTRA_CA_CERTS=%s
export NODE_USE_SYSTEM_CA=1

# Proxy routing (only when daemon is running — avoids ConnectionRefused)
if nc -z localhost %d 2>/dev/null; then
  export HTTP_PROXY=http://localhost:%d
  export HTTPS_PROXY=http://localhost:%d
fi
`, bundlePath, userCertPath, port, port, port)

	if err := os.WriteFile(envPath, []byte(envContent), 0o644); err != nil {
		fatal("write env.sh: %v", err)
	}
	step("Wrote %s", envPath)
	...
	if tokenFlag != "" && !noDaemon {
		...
		fmt.Println()
		fmt.Println("  Restart your shell (or run: source ~/.quint/env.sh)")
		fmt.Println("  Then verify: curl --proxy http://localhost:9090 https://httpbin.org/get")
	} else {
